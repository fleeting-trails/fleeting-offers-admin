  ...
  const handleKeyDown = (event: KeyboardEvent) => {
    if (event.key === 'Enter') {
      if (currentStep === 'email' && emailValid) {
        handleEmailNext()
      } else if (currentStep === 'verify' && formValid) {
        handleCreateAccount()
      }
    }
  }
...
>
  ...
  <div
    class="relative bg-gradient-to-b from-background-toned-1/95 dark:from-background-toned-1-dark/95 to-background-toned-0/95 dark:to-background-toned-0-dark/95 backdrop-blur-sm rounded-xl shadow-2xl border border-background-pure/20 dark:border-background-pure-dark/20 overflow-hidden max-w-xl w-full"
  >
